...
public:
  ...
  : Node("upper"), count_(0)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "SerialSender initialized");
  }
...
private:
  ...
  void sendTF()
  {
    ...
    // TODO: 填充tf_msg.transforms
    std::vector<uint8_t> data = {0x01}; // topic_id 0x01 for /tf
    appendData(data, "tf example");     // 这里你可以把tf_msg序列化成字节流
    sendPacket(data);
  }
  ...
  void sendScan()
  {
    ...
    // TODO: 填充scan_msg
    std::vector<uint8_t> data = {0x02}; // topic_id 0x02 for /scan
    appendData(data, "scan example");   // 这里你可以把scan_msg序列化成字节流
    sendPacket(data);
  }
  ...
  void sendMap()
  {
    ...
    // TODO: 填充map_msg
    std::vector<uint8_t> data = {0x03}; // topic_id 0x03 for /map
    appendData(data, "map example");    // 这里你可以把map_msg序列化成字节流
    sendPacket(data);
  }
  ...
  void sendPacket(const std::vector<uint8_t> &data)
  {
    ...
    packet.push_back(0xAA); // 帧头
    packet.insert(packet.end(), data.begin(), data.end());
    packet.push_back(0x0A); // 帧尾
...
ament_target_dependencies(upper rclcpp nav_msgs sensor_msgs tf2_msgs std_msgs)
...
<package format="3">
  ...
  <description>Serial sender for ROS 2 topics</description>
