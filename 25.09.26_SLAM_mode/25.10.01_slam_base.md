...
def generate_launch_description():
    serial_port = "/dev/ttyAMA0"  # 默认串口，可通过launch参数覆盖

    return LaunchDescription([
        Node(
            package='serial_sender',
            executable='serial_sender',
            name='serial_sender',
            parameters=[{'serial_port': serial_port, 'baud_rate': 921600}],
            output='screen'
        ),
        ExecuteProcess(
            cmd=['ros2', 'bag', 'play', '~/slam_ws/src/upper/bag/*.db3'],
            output='screen'
        ),
        ExecuteProcess(
            cmd=['rviz2', '-d', '~/slam_ws/src/lslidar_driver/rviz/lslidar.rviz'],
            output='screen'
        )
    ])
```
...
<package format="3">
  ...
  <maintainer email="you@example.com">cat</maintainer>
...
ros2 launch upper upper_launch.py serial_port:=/dev/ttyUSB0
