...
private:
  ...
  {
    sensor_msgs::msg::LaserScan scan_to_process;
    {
      std::lock_guard<std::mutex> lock(scan_mutex_);
      if (!has_new_scan_ || last_scan_.ranges.empty()) return;
      scan_to_process = last_scan_;
      has_new_scan_ = false;
    }

    auto filtered_scan = processScan(scan_to_process);
  ...
  sensor_msgs::msg::LaserScan processScan(const sensor_msgs::msg::LaserScan& input_scan)
  {
    ...
    bool has_intensities = !input_scan.intensities.empty() && input_scan.intensities.size() == original_size;
    if (has_intensities) {
      new_intensities.resize(target_size_, 0.0f);
    }
    ...
    for (int out_idx = 0; out_idx < target_size_; ++out_idx) {
      ...
      float src_index = (target_angle - original_angle_min) / original_angle_increment;
      
      int i0 = static_cast<int>(std::floor(src_index));
      int i1 = i0 + 1;
      ...
      if (i0 < 0) {
        ...
        i1 = 0;
        alpha = 0.0f;
      } else if (i1 >= static_cast<int>(original_size)) {
        i0 = original_size - 1;
        i1 = original_size - 1;
        alpha = 0.0f;
      }
      ...
      // 插值距离数据
      if (isValidMeasurement(input_scan.ranges[i0]) && isValidMeasurement(input_scan.ranges[i1])) {
        output_scan.ranges[out_idx] = (1.0f - alpha) * input_scan.ranges[i0] + alpha * input_scan.ranges[i1];
      } else if (isValidMeasurement(input_scan.ranges[i0])) {
        output_scan.ranges[out_idx] = input_scan.ranges[i0];
      } else if (isValidMeasurement(input_scan.ranges[i1])) {
        output_scan.ranges[out_idx] = input_scan.ranges[i1];
      } else {
        output_scan.ranges[out_idx] = std::numeric_limits<float>::infinity();
      }
      
      // 插值强度数据（如果有）
      if (has_intensities) {
        output_scan.intensities[out_idx] = (1.0f - alpha) * input_scan.intensities[i0] + alpha * input_scan.intensities[i1];
      }
    }
...
source ~/slam_ws/install/setup.bash
ros2 launch upper upper_launch.py serial_port:=/dev/ttyS3 baud_rate:=921600
```
