...
private:
  ...
  void publishFilteredScan()
  {
    ...
    std::vector<float> new_ranges(target_size_);
    std::vector<float> new_intensities(target_size_);

    for (int i = 0; i < target_size_; ++i)
    {
      float t = static_cast<float>(i) / (target_size_ - 1);
    ...
    filtered.angle_increment = total_angle / (target_size_ - 1);
