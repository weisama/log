...
public:
  ...
  : Node("lidar_filter")
  {
    ...
    RCLCPP_INFO(this->get_logger(), "LidarFilter started: target_size=%d, scan_topic=%s", target_size_, scan_topic_.c_str());
...
private:
  ...
  void publishFilteredScan()
  {
    ...
    // 如果 target_size 大于原始点数，直接使用原始大小
    int out_size = std::min(target_size_, n);

    std::vector<float> new_ranges(out_size);
    std::vector<float> new_intensities(out_size);

    for (int i = 0; i < out_size; ++i)
    {
      float t = static_cast<float>(i) / (out_size - 1);
    ...
    // 使用原始角度范围，计算新的 angle_increment
    filtered.angle_min = last_scan_.angle_min;
    filtered.angle_max = last_scan_.angle_max;
    filtered.angle_increment = (filtered.angle_max - filtered.angle_min) / (out_size - 1);
...
def generate_launch_description():
    ...
    # === 3. 启动 lslidar_driver ===
    driver_launch_file = os.path.join(get_package_share_directory('lslidar_driver'), 'launch', 'lslidar_launch.py')
