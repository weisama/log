...
### 6. Cartographer 建图，融合了rf2o_laser_odometry里程计
...
`gedit ~/slam_ws/src/karto_slam/config/my_async.yaml`
...
slam_toolbox:
  ros__parameters:
    ...
    # 求解器插件参数
    #######################################
    solver_plugin: solver_plugins::CeresSolver  # 使用Ceres求解器进行优化
    ceres_linear_solver: SPARSE_NORMAL_CHOLESKY  # 线性求解器类型，稀疏正态Cholesky分解
    ceres_preconditioner: SCHUR_JACOBI           # 预条件器类型，Schur补雅可比
    ceres_trust_strategy: LEVENBERG_MARQUARDT    # 信任区域策略，Levenberg-Marquardt算法
    ceres_dogleg_type: TRADITIONAL_DOGLEG        # Dogleg类型，传统Dogleg算法
    ceres_loss_function: None                    # 损失函数，None表示不使用
    ...
    # 地图起始位置设置（可选）
    #######################################
    # map_file_name: /path/to/your/map  # 加载已有地图的文件路径
    # map_start_pose: [0.0, 0.0, 0.0]   # 地图起始位姿[x, y, theta]
    # map_start_at_dock: false           # 是否从停靠点开始

    #######################################
    # 基本运行参数
    #######################################
    debug_logging: false          # 是否启用调试日志（会降低性能）
    throttle_scans: 1             # 扫描数据节流，1表示处理所有扫描数据
    transform_publish_period: 0.02 # 坐标变换发布周期(秒)，0表示不发布里程计
    map_update_interval: 2.0      # 地图更新间隔(秒)，较小的值会得到更实时的地图但计算量更大
    resolution: 0.025             # 地图分辨率(米/像素)，值越小地图越精细但占用内存越大
    min_laser_range: 0.1          # 激光最小有效距离(米)，忽略小于此值的雷达数据
    max_laser_range: 12.0         # 激光最大有效距离(米)，忽略大于此值的雷达数据
    minimum_time_interval: 0.5    # 处理扫描数据的最小时间间隔(秒)
    transform_timeout: 0.1        # 坐标变换查询超时时间(秒)
    tf_buffer_duration: 10.0      # TF缓冲区持续时间(秒)
    stack_size_to_use: 40000000   # 堆栈大小(字节)，处理大地图时需要更大的堆栈
    enable_interactive_mode: true # 是否启用交互模式，允许实时调整参数

    #######################################
    # 扫描匹配和运动检测参数
    #######################################
    use_scan_matching: true       # 是否使用扫描匹配
    use_scan_barycenter: false    # 是否使用扫描质心，对于2D SLAM通常设为false
    minimum_travel_distance: 0.1  # 触发处理的最小移动距离(米)，值越小地图更新越频繁
    minimum_travel_heading: 0.1   # 触发处理的最小转动角度(弧度)，值越小地图更新越频繁
    scan_buffer_size: 20          # 扫描缓冲区大小，存储历史扫描数据用于匹配
    scan_buffer_maximum_scan_distance: 5.0  # 扫描缓冲区中扫描数据的最大距离(米)
    
    #######################################
    # 局部匹配参数（相邻扫描匹配）
    #######################################
    link_match_minimum_response_fine: 0.1   # 精细匹配的最小响应值，值越小匹配要求越低
    link_scan_maximum_distance: 2.0         # 局部匹配的最大搜索距离(米)
    
    #######################################
    # 闭环检测参数
    #######################################
    do_loop_closing: true                    # 是否启用闭环检测
    loop_search_maximum_distance: 5.0        # 闭环搜索的最大距离(米)
    loop_match_minimum_chain_size: 3         # 闭环检测的最小链长度，值越小检测越敏感
    loop_match_maximum_variance_coarse: 1.0  # 粗匹配的最大方差阈值
    loop_match_minimum_response_coarse: 0.7  # 粗匹配的最小响应值
    loop_match_minimum_response_fine: 0.8    # 精细匹配的最小响应值

    #######################################
    # 相关性参数 - 局部匹配
    #######################################
    correlation_search_space_dimension: 1.0      # 相关性搜索空间维度(米)
    correlation_search_space_resolution: 0.01    # 相关性搜索空间分辨率(米)
    correlation_search_space_smear_deviation: 0.05 # 相关性搜索空间平滑偏差(米)

    #######################################
    # 相关性参数 - 闭环检测
    #######################################
    loop_search_space_dimension: 6.0         # 闭环搜索空间维度(米)
    loop_search_space_resolution: 0.05       # 闭环搜索空间分辨率(米)
    loop_search_space_smear_deviation: 0.03  # 闭环搜索空间平滑偏差(米)

    #######################################
    # 扫描匹配器参数
    #######################################
    distance_variance_penalty: 0.3      # 距离方差惩罚因子，值越小对距离变化越敏感
    angle_variance_penalty: 0.5         # 角度方差惩罚因子，值越小对角度变化越敏感
    fine_search_angle_offset: 0.001     # 精细搜索角度偏移(弧度)
    coarse_search_angle_offset: 0.2     # 粗搜索角度偏移(弧度)
    coarse_angle_resolution: 0.03       # 粗搜索角度分辨率(弧度)
    minimum_angle_penalty: 0.8          # 最小角度惩罚阈值
    minimum_distance_penalty: 0.7       # 最小距离惩罚阈值
    use_response_expansion: true        # 是否使用响应扩展
    
    #######################################
    # 地图构建参数
    #######################################
    min_pass_through: 1          # 最小通过次数，影响障碍物判断的敏感度
    occupancy_threshold: 0.1     # 占据阈值，用于判断单元格是否被占据
```
