...
def generate_launch_description():
    ...
    mode = LaunchConfiguration('mode')                 # 'mapping' 或 'localization'
    sync_async = LaunchConfiguration('sync_async')     # 'sync' 或 'async'
    ...
    declare_scan_topic_cmd = DeclareLaunchArgument(
        'scan_topic', default_value='/scan', description='Laser scan topic name')
    
    declare_mode_cmd = DeclareLaunchArgument(
        'mode', default_value='mapping', description='SLAM mode: mapping or localization')
    
    declare_sync_async_cmd = DeclareLaunchArgument(
        'sync_async', default_value='async', description='Run SLAM in sync or async mode')

    # 根据参数选择节点执行文件
    slam_executable = Node(
        parameters=[
            slam_params_file,
            {
        ...
        executable=[sync_async, '_slam_toolbox_node'],  # 会拼接成 'async_slam_toolbox_node' 或 'sync_slam_toolbox_node'
        name='slam_toolbox',
        output='screen'
    )
...
slam_toolbox:
  ros__parameters: # Plugin params
    ...
    use_odom: true                # 启用里程计，SLAM_TOOLBOX可以只订阅话题，自身会发布 map -> odom -> base_link

    # 运行模式选择
    mode: localization  # 建图模式（可选：mapping / localization），可通过 launch 参数覆盖
    sync_async: sync        # 默认 sync，可通过 launch 参数覆盖
