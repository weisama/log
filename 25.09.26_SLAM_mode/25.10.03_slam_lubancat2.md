...
def generate_launch_description():
    ...
    # 参数文件路径
    slam_params_file = os.path.join(get_package_share_directory("karto_slam"),
                                    'config', 'my_parameter.yaml')

    # 拼接 executable
    slam_executable_name = sync_async.perform(None) + '_slam_toolbox_node'

    # SLAM Toolbox 节点
    slam_node = Node(
        package='slam_toolbox',
        executable=slam_executable_name,
        name='slam_toolbox',
        output='screen',
        parameters=[
            slam_params_file,
