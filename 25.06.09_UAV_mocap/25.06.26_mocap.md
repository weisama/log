...
cp -r ~/vrpn_client_ros2/src/vrpn_listener ~/ws_livox/src
```

连接设置

```bash
gedit ~/ws_livox/src/vrpn_listener/config/params.yaml
```
...
map 坐标系 (全局地图坐标系)，odom 坐标系 (里程计坐标系)，ENU (East-North-Up)  
X轴指向东，Y轴指向北，Z轴指向上  
动捕标定采用此坐标系  
刚体创建采用此坐标系
/mavros/odometry/out准寻此坐标系，mavros会自动将/mavros/odometry/out的ENU转化为NED给飞控
...
骨骼朝向x，即机头方向，指向水平面  
创建后校验：刚体xyz等同于动捕坐标系xyz，机头为刚体x，左侧为刚体y，上为刚体z
...
cd ~/ws_livox/src/vrpn_relative_pose
gedit ~/ws_livox/src/vrpn_relative_pose/src/vrpn_relative_pose_node.cpp
